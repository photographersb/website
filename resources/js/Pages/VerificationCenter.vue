    ...
    <div class="max-w-6xl mx-auto">
      ...
      <div class="mb-8 sm:mb-12">
        <div class="flex items-start sm:items-center gap-3 sm:gap-4 mb-4">
          <div class="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-gradient-to-br from-burgundy-500 to-burgundy-700 flex items-center justify-center shadow-xl">
          ...
          <div>
            <h1 class="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900">Verification <span class="text-burgundy-600">Center</span></h1>
            <p class="text-sm sm:text-base lg:text-lg text-burgundy-600 font-semibold mt-1">Build trust with verified credentials</p>
          </div>
        </div>
        <div class="h-1 w-24 bg-gradient-to-r from-burgundy-500 to-burgundy-400 rounded-full"></div>
      </div>

      <div v-if="loading" class="flex justify-center items-center py-20">
        <div class="text-center">
          <div class="animate-spin rounded-full h-16 w-16 border-4 border-burgundy-200 border-t-burgundy-600 mx-auto mb-4"></div>
          <p class="text-burgundy-600 font-semibold">Loading your verification status...</p>
        </div>
      </div>

      <div v-else>
        <!-- Not Photographer Alert -->
        <div v-if="!isPhotographer" class="mb-6 sm:mb-8 bg-white rounded-2xl shadow-lg p-5 sm:p-8 border-l-4 border-burgundy-600">
          <div class="flex items-start gap-4">
            <div class="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-burgundy-100 flex items-center justify-center flex-shrink-0">
              <svg class="w-7 h-7 text-burgundy-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <div class="flex-1">
              <h3 class="text-xl font-bold text-burgundy-700 mb-2">Photographer Account Required</h3>
              <p class="text-gray-600 mb-4">Verification is exclusively for photographer accounts. To get started with verification, please create a photographer profile first.</p>
              <router-link to="/photographer/profile" class="inline-flex w-full sm:w-auto items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-burgundy-600 to-burgundy-700 text-white rounded-lg hover:from-burgundy-700 hover:to-burgundy-800 transition-all font-semibold shadow-md hover:shadow-lg">
        ...
        <div v-else class="space-y-8">
          <!-- Status Overview -->
          <div v-if="statusItems.length" class="bg-white rounded-2xl shadow-xl p-5 sm:p-8 border border-gray-100">
            <div class="flex items-center gap-3 mb-6 sm:mb-8">
              <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-burgundy-100 to-burgundy-200 flex items-center justify-center">
                <svg class="w-6 h-6 text-burgundy-700" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd" d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 3.062v6.757a1 1 0 01-.940 1.017 48.412 48.412 0 01-7.125 0 1 1 0 01-.94-1.017v-6.757a3.066 3.066 0 012.812-3.062zM9 12a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd" />
                </svg>
              </div>
              <h2 class="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-gray-900 to-burgundy-700 bg-clip-text text-transparent">Your Verification Status</h2>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              ...
              >
                <div class="flex items-start justify-between mb-4">
                  <div>
                    <p class="text-sm font-bold text-gray-700 uppercase tracking-wide">{{ formatType(item.type) }}</p>
                    <span
                      :class="[
                ...
                <div class="space-y-3 text-sm text-gray-700 border-t border-gray-200 pt-4">
                  ...
                  <p v-if="item.expires_at" :class="isExpired(item.expires_at) ? 'text-orange-700 font-bold' : ''">
                    <span class="font-semibold" :class="isExpired(item.expires_at) ? 'text-orange-700' : 'text-burgundy-700'">📅 Expires:</span>
            ...
            <div v-if="pendingRequests" class="mt-8 p-5 bg-gradient-to-r from-burgundy-50 to-rose-50 border-l-4 border-burgundy-600 rounded-lg">
          ...
          <!-- No Verifications -->
          <div v-else class="bg-gradient-to-br from-burgundy-50 via-rose-50 to-burgundy-50 rounded-2xl shadow-lg p-6 sm:p-10 border-2 border-burgundy-200 text-center">
            <div class="w-20 h-20 rounded-full bg-gradient-to-br from-burgundy-100 to-burgundy-200 flex items-center justify-center mx-auto mb-5 shadow-md">
              <svg class="w-10 h-10 text-burgundy-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4" />
              </svg>
            </div>
            <h3 class="text-xl font-bold text-burgundy-900 mb-3">No Verifications Yet</h3>
            <p class="text-burgundy-700 font-medium mb-4">Start building credibility by submitting verification requests below.</p>
          </div>
          ...
          <div class="bg-white rounded-2xl shadow-2xl p-5 sm:p-8 border border-gray-100">
            <div class="flex items-start sm:items-center gap-4 mb-6 sm:mb-8 pb-5 sm:pb-6 border-b-2 border-gray-100">
              ...
              <div>
                ...
                <p class="text-burgundy-700 text-sm mt-2 font-semibold">Add verified credentials to strengthen your profile</p>
              </div>
            </div>

            <p class="text-gray-700 mb-8 text-base font-medium leading-relaxed">Choose a verification type and submit supporting documents. Our team will review your submission within <span class="text-burgundy-700 font-bold">24-48 hours</span>.</p>

            <form id="verificationForm" @submit.prevent="submitRequest" class="space-y-6 sm:space-y-8">
              <div>
                ...
                >
                  ...
                  <option value="studio_address">🏢 Studio Address Proof</option>
                </select>
              ...
              <div>
                ...
                <div
                  ...
                  :class="[
                    'relative border-3 border-dashed rounded-2xl p-6 sm:p-10 text-center transition-all cursor-pointer',
                    dragActive
                      ? 'border-burgundy-600 bg-burgundy-50'
                      : 'border-burgundy-300 bg-gradient-to-br from-burgundy-50 to-rose-50 hover:border-burgundy-500 hover:bg-burgundy-100/50'
                  ]"
                >
                  <input
                    type="file"
                  ...
                  <div @click="$refs.fileInput?.click()" class="cursor-pointer">
                    ...
                    <p class="text-lg sm:text-xl font-extrabold text-burgundy-900 mb-2">Click to upload or drag and drop</p>
                    <p class="text-sm sm:text-base text-burgundy-800 font-bold mt-2 bg-white/60 inline-block px-4 py-2 rounded-lg">JPG, PNG, PDF (Max 5 files, 10MB each)</p>
                  </div>
                </div>
                    ...
                    >
                      <div class="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div class="flex-1 min-w-0">
                          ...
                          <p class="text-xs text-burgundy-700 font-medium">{{ (file.size / 1024).toFixed(1) }} KB</p>
                        </div>
                      </div>
                      <button
                        type="button"
              ...
              <transition name="fade">
                ...
                >
                  <div class="flex items-start gap-3">
                    ...
                    <p class="text-sm text-green-900 font-semibold">{{ message }}</p>
                  </div>
                </div>
              </transition>
              ...
              <transition name="fade">
                ...
                >
                  <div class="flex items-start gap-3">
                    ...
                    <p class="text-sm text-red-900 font-semibold">{{ error }}</p>
                  </div>
                </div>
              </transition>
            </form>
          ...
          <div v-if="isPhotographer" class="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur border-t border-gray-200 px-4 py-3 z-40">
            <button
...
const addFiles = (newFiles) => {
  const validFiles = newFiles.filter(file => {
    if (file.size > 10 * 1024 * 1024) {
      error.value = `File ${file.name} is too large. Maximum 10MB allowed.`
      return false
    }
    if (!['image/jpeg', 'image/png', 'application/pdf'].includes(file.type)) {
      error.value = `File ${file.name} has an invalid format. Only JPG, PNG, and PDF are allowed.`
      return false
    }
    return true
  })

  files.value = [...files.value, ...validFiles].slice(0, 5)
}
...
onMounted(async () => {
  try {
    ...
    console.error('Failed to initialize verification center:', e)
  } finally {
    loading.value = false
  }
})
