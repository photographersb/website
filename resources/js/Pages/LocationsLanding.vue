    ...
    <div class="max-w-7xl mx-auto">
      <div class="mb-12">
        <div class="flex items-center gap-4 mb-4">
          ...
          <div>
            <h1 class="text-5xl font-bold text-gray-900">Explore <span class="text-primary-700">Locations</span></h1>
            <p class="text-lg text-primary-700 font-semibold mt-1">Discover photographers in cities across Bangladesh</p>
          </div>
        </div>
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p class="text-gray-600">Select a city to view available photographers.</p>
          </div>
      ...
      <div v-if="loading" class="flex justify-center items-center py-20">
        <div class="text-center">
          ...
          <p class="text-primary-700 font-semibold">Loading locations...</p>
        </div>
      </div>

      <div v-else>
        <div v-if="filteredCities.length === 0" class="bg-white rounded-2xl shadow-lg p-12 border border-gray-100 text-center">
          <div class="text-6xl mb-4">📍</div>
          <h3 class="text-2xl font-bold text-gray-900 mb-2">No Cities Found</h3>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            ...
            <p class="text-gray-600 mb-4">Browse photographers in {{ city.name }}</p>
...
const loadCities = async () => {
  try {
    ...
    const citiesWithCounts = await Promise.all(allCities.map(async (city) => {
      try {
        const countResponse = await api.get(`/v1/photographers?city=${city.slug}&per_page=1`)
