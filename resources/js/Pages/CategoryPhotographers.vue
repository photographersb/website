    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="mb-8 sm:mb-12">
        <div class="flex items-start sm:items-center gap-3 sm:gap-4 mb-4">
          ...
          <div>
            <h1 class="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900">Find Photographers <span class="text-primary-700">by Category</span></h1>
            <p class="text-sm sm:text-base lg:text-lg text-primary-700 font-semibold mt-1">Browse by photography specialty</p>
          </div>
        </div>
      ...
      <!-- Category Grid -->
      <div v-if="!selectedCategory" class="mb-10 sm:mb-12">
        <h2 class="text-xl sm:text-2xl font-bold text-gray-900 mb-5 sm:mb-6">Photography Categories</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          >
            <div class="text-5xl mb-4 group-hover:scale-125 transition-transform">{{ category.icon }}</div>
      ...
      <button
        ...
        class="mb-6 inline-flex items-center gap-2 px-4 py-2 text-primary-700 hover:text-primary-800 font-semibold hover:bg-primary-100 rounded-lg transition-all"
      >
      ...
      <div v-if="selectedCategory && loading" class="flex justify-center items-center py-20">
        <div class="text-center">
          <div class="animate-spin rounded-full h-16 w-16 border-4 border-primary-200 border-t-primary-600 mx-auto mb-4"></div>
          <p class="text-primary-700 font-semibold">Loading photographers...</p>
        </div>
      </div>

      <!-- Category View -->
      <div v-else-if="selectedCategory" class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
        <!-- Filters Sidebar -->
        <div class="hidden lg:block lg:col-span-4">
          <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 sticky top-20">
            <!-- Selected Category Info -->
            <div class="mb-8 pb-8 border-b border-gray-200">
              <div class="text-4xl mb-3">{{ selectedCategoryData?.icon }}</div>
              <h3 class="text-xl font-bold text-gray-900">{{ selectedCategoryData?.name }}</h3>
              <p class="text-sm text-gray-600 mt-2">{{ selectedCategoryData?.description }}</p>
            </div>
            ...
            <div class="mb-8">
              ...
              <div class="space-y-2">
                <button
                  ...
                  :class="[
                    'w-full text-left px-4 py-3 rounded-lg font-bold transition-all',
                    !selectedPrice
            ...
            <div class="mb-8 pb-8 border-b border-gray-200">
              ...
              <div class="space-y-2">
                ...
                <button
                  ...
                  :class="[
                    'w-full text-left px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-between',
        ...
        <div class="lg:col-span-8">
          ...
          <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="flex items-center justify-between gap-3">
              ...
              <button
                class="lg:hidden px-4 py-2 rounded-lg border-2 border-primary-300 text-primary-700 font-bold hover:bg-primary-50"
          ...
          <div class="mb-4 sm:mb-6 flex items-center justify-between">
            <p class="hidden sm:block text-lg font-bold text-gray-900">
              Found <span class="text-primary-700">{{ filteredPhotographers.length }}</span> photographers
            </p>
          ...
          <!-- Empty State -->
          <div v-if="filteredPhotographers.length === 0" class="bg-white rounded-2xl shadow-lg p-6 sm:p-12 border border-gray-100 text-center">
            <div class="text-6xl mb-4">🎯</div>
            <h3 class="text-2xl font-bold text-gray-900 mb-2">No Photographers Found</h3>
            <p class="text-gray-600 mb-6">Try changing your filters</p>
            <button
              @click="selectedPrice = null; selectedRating = null; sortBy = 'recent'"
              class="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-primary-700 to-primary-800 text-white rounded-lg hover:from-primary-800 hover:to-primary-900 transition-all font-semibold"
            >
              Clear Filters
            </button>
          </div>
          ...
          <div v-else class="space-y-3 sm:space-y-4">
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <!-- Description -->
                <p class="text-gray-600 text-sm mb-4">{{ photographer.bio || 'Professional photographer' }}</p>
                ...
                <div class="flex flex-col sm:flex-row gap-2">
                  ...
                  <button class="w-full sm:w-auto px-6 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-all font-semibold text-sm border border-gray-200">
                    Message
                  </button>
                </div>
              </div>
          ...
          <!-- Load More -->
          <div v-if="filteredPhotographers.length > itemsPerPage" class="mt-8 text-center">
            <button
      ...
      >
        <div class="space-y-6">
          <div>
            <div class="text-sm font-bold text-gray-500 uppercase tracking-wide mb-2">Category</div>
            <div class="flex items-center gap-3">
              <div class="text-2xl">{{ selectedCategoryData?.icon }}</div>
              <div>
                <p class="text-base font-bold text-gray-900">{{ selectedCategoryData?.name }}</p>
                <p class="text-sm text-gray-600">{{ selectedCategoryData?.description }}</p>
              </div>
            </div>
          </div>
...
const loadCategories = async () => {
  try {
    ...
    // Load photographer counts for each category
    const categoriesWithCounts = await Promise.all(allCategories.map(async (cat) => {
      try {
        const countResponse = await api.get(`/v1/photographers?category=${cat.slug}&per_page=1`)
