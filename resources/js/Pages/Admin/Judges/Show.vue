    ...
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div class="max-w-4xl mx-auto">
        <div v-if="loading" class="bg-white rounded-lg shadow p-12 text-center">
          ...
          <p class="text-gray-600">Loading judge details...</p>
        </div>

        <div v-else-if="judge.id" class="bg-white rounded-lg shadow overflow-hidden">
          <!-- Header Section -->
          <div class="bg-gradient-to-r from-purple-50 to-indigo-50 p-6 border-b">
            <div class="flex items-start gap-6">
              <div v-if="judge.profile_image_url" class="flex-shrink-0">
                <img :src="judge.profile_image_url" :alt="judge.name" class="h-32 w-32 rounded-lg object-cover border-4 border-white shadow-lg" />
              </div>
              <div class="flex-1">
                <div class="flex items-start justify-between">
                  <div>
                    <h1 class="text-3xl font-bold text-gray-900">{{ judge.name }}</h1>
                    <p v-if="judge.title" class="text-lg text-gray-600 mt-1">{{ judge.title }}</p>
                    <p v-if="judge.organization" class="text-gray-500 mt-1">{{ judge.organization }}</p>
                  </div>
          ...
          <div class="p-6 space-y-6">
            <!-- Bio Section -->
            <div v-if="judge.bio" class="border-b pb-6">
              <h2 class="text-xl font-semibold text-gray-900 mb-3">Biography</h2>
              <p class="text-gray-700 whitespace-pre-line">{{ judge.bio }}</p>
            </div>
            ...
            <!-- Social Media -->
            <div v-if="judge.facebook_url || judge.instagram_url || judge.website_url" class="border-b pb-6">
              <h3 class="text-lg font-semibold text-gray-900 mb-3">Social Media & Web</h3>
              <div class="flex flex-wrap gap-3">
                <a v-if="judge.facebook_url" :href="judge.facebook_url" target="_blank" 
                ...
                <a v-if="judge.instagram_url" :href="judge.instagram_url" target="_blank" 
                ...
                <a v-if="judge.website_url" :href="judge.website_url" target="_blank" 
          ...
          <div class="bg-gray-50 px-6 py-4 flex justify-between items-center border-t">
            ...
            <div class="flex gap-3">
              <button @click="$router.push(`/admin/judges/${judge.id}/edit`)" 
              ...
              <button @click="deleteJudge" class="px-4 py-2 text-red-700 bg-red-50 border border-red-200 rounded-lg hover:bg-red-100">
                Delete
              </button>
            </div>
          </div>
        ...
        <div v-else class="bg-white rounded-lg shadow p-12 text-center">
          <p class="text-gray-600">Judge not found</p>
        </div>
      </div>
    </div>
...
const fetchJudge = async () => {
  try {
    ...
    const response = await fetch(`/api/v1/admin/judges/${judgeId.value}`, {
...
const toggleStatus = async () => {
  try {
    const token = localStorage.getItem('auth_token')
    const response = await fetch(`/api/v1/admin/judges/${judgeId.value}/toggle-status`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
    })
    const data = await response.json()
