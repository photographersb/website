    ...
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      >
        ...
        <div class="bg-white rounded-lg shadow-card overflow-hidden">
          <div
            v-if="competition.hero_image"
            class="h-64 bg-gray-200"
          >
            <img
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
          <div class="md:col-span-2 bg-gradient-to-br from-burgundy via-red-700 to-red-800 rounded-2xl shadow-xl p-6 md:p-8 text-white">
            <div class="text-3xl md:text-4xl font-black mb-2">
              {{ formatNumber(Math.floor(competition.total_prize_pool)) }}
            </div>
            <div class="text-xs md:text-sm text-white/90 font-medium">
              Prize Pool
            </div>
          </div>
          ...
          <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-2xl shadow-xl p-6 md:p-8 text-white">
            <div class="text-2xl md:text-3xl font-black mb-2">
              {{ formatNumber(competition.entry_fee) }}
            </div>
            <div class="text-xs md:text-sm text-white/90 font-medium">
              Entry Fee
            </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div class="text-gray-900 mt-1">
                {{ formatDate(competition.submission_start_date) }}
              </div>
            </div>
            <div>
              <div class="text-sm font-medium text-gray-600">
            ...
            <div>
              ...
              <div class="text-gray-900 mt-1">
                {{ formatDate(competition.voting_start_date) }}
              </div>
            </div>
            <div>
              <div class="text-sm font-medium text-gray-600">
              ...
              <div class="text-gray-900 mt-1">
                {{ formatDate(competition.voting_end_date) }}
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="bg-white rounded-lg shadow-card p-6">
            ...
            <div class="space-y-3 text-sm">
              ...
              <div class="flex items-center gap-2">
                ...
                <span class="text-gray-900">{{ Math.round((Number(competition.vote_weight ?? 0.4) * 100)) }}%</span>
              </div>
              <div class="flex items-center gap-2">
                <span class="text-gray-600 font-medium">Judge Score Weight:</span>
                <span class="text-gray-900">{{ Math.round((Number(competition.judge_weight ?? 0.6) * 100)) }}%</span>
              </div>
              <div
                v-if="competition.judging_criteria"
                class="mt-4"
              >
                <div class="text-gray-600 font-medium mb-2">
                  Criteria:
        ...
        <!-- Judges -->
        <div
          v-if="competition.judgeProfiles && competition.judgeProfiles.length > 0"
          class="bg-white rounded-lg shadow-card p-6"
        >
          <h2 class="text-xl font-bold text-gray-900 mb-4">
            Judges
          </h2>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div
              v-for="judge in competition.judgeProfiles"
              :key="judge.id"
            ...
            >
              <div class="flex items-center gap-3">
                ...
                <div
                  v-else
                  class="w-12 h-12 rounded-full bg-burgundy-100 flex items-center justify-center font-bold text-burgundy-600"
                >
                  {{ judge.name.charAt(0) }}
                </div>
                <div>
                  ...
                  <div
                    ...
                    class="text-xs text-burgundy-600"
                  >
                    {{ judge.expertise }}
                  </div>
                </div>
        ...
        >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div
              v-for="sponsor in competition.sponsors"
              :key="sponsor.id"
              class="border border-gray-200 rounded-lg p-4 flex items-start gap-4"
            >
              <img
              ...
              <div
                v-else
                class="w-16 h-16 bg-gray-200 rounded flex items-center justify-center text-gray-600 font-bold"
              >
...
export default {
  ...
  setup() {
    ...
    const fetchCompetition = async () => {
      try {
        loading.value = true;
        const response = await api.get(`/admin/competitions/${competitionId.value}`);
    ...
    const deleteCompetition = async () => {
      ...
      try {
        const response = await api.delete(`/admin/competitions/${competitionId.value}`);
