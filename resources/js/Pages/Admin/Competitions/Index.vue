<template>
  <div class="min-h-screen bg-gray-50 competitions-page">
    ...
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div class="dashboard-grid">
        ...
        <div class="dashboard-card full-width">
          <div class="card-header">
            ...
            <router-link
              to="/admin/competitions?status=active"
              class="card-link"
            >
              View All →
            </router-link>
              ...
              <div class="competition-body">
                <div class="competition-meta">
                  ...
                  <div class="meta-item">
                    ...
                    <span>Prize pool: ৳{{ formatNumber(comp.total_prize_pool || 0) }}</span>
                  </div>
                </div>
                <div class="competition-chips">
                  <span class="chip">
                    Fee: ৳{{ formatNumber(comp.participation_fee || 0) }}
                  </span>
                  <span class="chip">
                    Deadline: {{ formatDate(comp.submission_deadline) }}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div class="dashboard-card full-width">
          <div class="card-header">
            ...
            <router-link
              to="/admin/competitions?status=draft"
              class="card-link"
            >
              View All →
            </router-link>
          ...
          <div
            v-else-if="draftCompetitions.length > 0"
            class="competitions-grid"
          >
            <div
          ...
          >
            ...
            >
              <div class="submission-info">
                ...
                <div class="submission-competition">
            ...
            >
              <div class="deadline-info">
                ...
                <div class="deadline-date">
                  {{ formatDate(comp.submission_deadline) }}
                </div>
              </div>
              <div class="deadline-countdown">
                {{ getDaysUntil(comp.submission_deadline) }} days
              </div>
            </div>
          </div>
          <div
      ...
      <div class="content-card">
        <div class="card-header">
          <h2 class="card-title">
            All Competitions ({{ competitions.total || 0 }})
          </h2>
          <button
            class="btn-export"
            @click="exportCompetitions"
          >
            <svg
              class="w-5 h-5 mr-2"
              fill="none"
        ...
        <!-- Competitions Table -->
        <div
          v-if="!loading && competitions.data && competitions.data.length > 0"
        ...
        <!-- Empty State -->
        <div
          v-else-if="!loading && (!competitions.data || competitions.data.length === 0)"
          class="empty-state"
        >
  ...
  methods: {
    ...
    async fetchCompetitions() {
      try {
        ...
        if (this.form.category_id) params.category_id = this.form.category_id;
        if (this.form.featured !== '') params.featured = this.form.featured;
        if (this.form.date_filter) params.date_filter = this.form.date_filter;
...
.competitions-page {
  --card-bg: #ffffff;
  --card-border: rgba(106, 24, 24, 0.08);
  --card-shadow: 0 10px 30px rgba(32, 20, 20, 0.08);
  --chip-bg: #f7efe9;
  --chip-text: #4b2f2f;
  --accent-red: #7c0a2e;
  --accent-gold: #b8842a;
  --accent-forest: #2f6b3f;
  --accent-ocean: #1f5f7a;
}
...
.stat-card::after {
  ...
  position: absolute;
  inset: 0;
  background: linear-gradient(135deg, rgba(124, 10, 46, 0.08), rgba(255, 255, 255, 0));
  pointer-events: none;
}

.stat-card.stat-blue::after {
  background: linear-gradient(135deg, rgba(31, 95, 122, 0.18), rgba(255, 255, 255, 0));
}

.stat-card.stat-green::after {
  background: linear-gradient(135deg, rgba(47, 107, 63, 0.18), rgba(255, 255, 255, 0));
}

.stat-card.stat-yellow::after {
  background: linear-gradient(135deg, rgba(184, 132, 42, 0.2), rgba(255, 255, 255, 0));
}

.stat-card.stat-purple::after {
  background: linear-gradient(135deg, rgba(124, 10, 46, 0.16), rgba(255, 255, 255, 0));
}
...
.competition-card:hover {
  ...
  box-shadow: 0 16px 32px rgba(32, 20, 20, 0.12);
}
