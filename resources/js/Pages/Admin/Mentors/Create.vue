          ...
          >
            ...
            <div class="flex justify-end gap-3 pt-4 border-t">
              ...
              <button
                type="submit"
                :disabled="saving"
                class="px-4 py-2 text-white bg-var(--admin-brand-primary) rounded-lg hover:bg-opacity-90"
...
const saveMentor = async () => {
  ...
  try {
    ...
    const { data } = await api.post('/admin/mentors', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })

    if (data.status === 'success') {
      toastMessage.value = 'Mentor created successfully!'
      ...
      generalError.value = data.message || 'Error creating mentor'
    }
  } catch (error) {
    console.error('Error saving mentor', error)
    generalError.value = 'Error creating mentor. Please try again.'
  } finally {
    saving.value = false
  }
}
