  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-gray-600 mt-1">Manage certificate designs for competitions and awards</p>
      </div>
      <button
    ...
    <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="i in 6" :key="i" class="bg-white rounded-lg shadow p-6 h-64 animate-pulse">
    ...
    <div v-else-if="templates.length === 0" class="bg-white rounded-lg shadow p-12 text-center">
      ...
      <p class="text-gray-600 mb-6">Create your first certificate template</p>
      <button
        @click="showCreateModal = true"
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        ...
        <div class="aspect-video bg-gradient-to-br from-orange-100 to-orange-50 relative overflow-hidden">
          <div class="w-full h-full flex items-center justify-center p-4 border-2 border-dashed border-orange-300">
            <div class="text-center">
              <p class="text-sm font-serif font-bold text-orange-600">{{ template.title }}</p>
              <p class="text-xs text-gray-600 mt-1">{{ template.type }}</p>
              <p class="text-xs text-gray-500 mt-2">{{ template.width }}×{{ template.height }}mm</p>
            </div>
          </div>
          <!-- Overlay Actions -->
        ...
        <!-- Template Info -->
        <div class="p-4">
          <h3 class="font-semibold text-gray-900 truncate">{{ template.title }}</h3>
          ...
          <div class="mt-3 flex items-center justify-between">
            ...
            <span v-if="template.is_default" class="px-2 py-1 bg-green-100 text-green-800 rounded text-xs font-medium">
              Default
            </span>
          </div>
        </div>
...
const loadTemplates = async () => {
  loading.value = true;
  try {
    const { data } = await api.get('/api/v1/admin/certificate-templates');
  ...
  try {
    ...
    if (editingTemplate.value?.id) {
      ...
      response = await api.put(
        `/api/v1/admin/certificate-templates/${editingTemplate.value.id}`,
        templateData
      );
    } else {
      // Create new
...
const deleteTemplate = async (template) => {
  ...
  try {
    const { data } = await api.delete(`/api/v1/admin/certificate-templates/${template.id}`);
    if (data.status === 'success') {
