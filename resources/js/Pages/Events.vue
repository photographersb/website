  ...
  <div class="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <div class="container mx-auto px-4 py-16 md:py-24 relative z-10">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mt-10 animate-fade-in-delay-2">
          <div class="bg-white/10 backdrop-blur-sm rounded-xl p-6 text-center border border-white/20 hover:bg-white/15 transition-all">
            <div class="text-3xl md:text-4xl font-bold">{{ stats.total_events || 0 }}</div>
            <div class="text-sm md:text-base text-gray-200 mt-1">Total Events</div>
          </div>
          <div class="bg-white/10 backdrop-blur-sm rounded-xl p-6 text-center border border-white/20 hover:bg-white/15 transition-all">
            <div class="text-3xl md:text-4xl font-bold">{{ stats.upcoming_events || 0 }}</div>
            <div class="text-sm md:text-base text-gray-200 mt-1">Upcoming</div>
          </div>
          <div class="bg-white/10 backdrop-blur-sm rounded-xl p-6 text-center border border-white/20 hover:bg-white/15 transition-all">
            <div class="text-3xl md:text-4xl font-bold">{{ stats.total_cities || 0 }}</div>
            <div class="text-sm md:text-base text-gray-200 mt-1">Cities</div>
          </div>
          <div class="bg-white/10 backdrop-blur-sm rounded-xl p-6 text-center border border-white/20 hover:bg-white/15 transition-all">
    ...
    <div class="container mx-auto px-3 sm:px-4 md:px-6 py-6 sm:py-8 md:py-12">
      ...
      <div class="bg-white rounded-lg shadow-md p-4 sm:p-5 md:p-6 mb-6 sm:mb-8">
        <h2 class="text-lg sm:text-xl font-semibold mb-4">Filter Events</h2>
      ...
      <!-- Empty State -->
      <div v-else-if="events.length === 0" class="text-center py-16 sm:py-20 md:py-24">
        ...
        <h3 class="text-lg sm:text-xl md:text-2xl font-semibold text-gray-800 mb-2">No Events Found</h3>
        <p class="text-sm sm:text-base text-gray-600">Try adjusting your filters or check back later for new events</p>
      </div>
      ...
      <div v-else>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
          ...
          >
            <!-- Event Image -->
            <div class="relative h-48 sm:h-52 md:h-56 overflow-hidden">
              <img
                :src="event.hero_image_url || 'https://via.placeholder.com/600x400?text=Event'"
                :alt="event.title"
                class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300"
              />
              <div v-if="event.is_featured" class="absolute top-3 right-3 bg-yellow-400 text-yellow-900 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-semibold">
                ⭐ Featured
              </div>
              <div v-if="event.is_ticketed" class="absolute top-3 left-3 bg-burgundy text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-semibold">
                Ticketed
              </div>
            </div>
            ...
            <div class="p-4 sm:p-5">
              ...
              <div class="space-y-2 mb-4">
                ...
                <!-- Price (if ticketed) -->
                <div v-if="event.is_ticketed" class="flex items-center text-xs sm:text-sm text-gray-700">
                  <svg class="w-4 h-4 mr-2 text-burgundy" fill="none" stroke="currentColor" viewBox="0 0 24 24">
