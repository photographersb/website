<template>
  <transition name="notification">
    <div v-if="show" :class="notificationClasses" class="fixed top-4 right-4 z-50 max-w-md shadow-2xl rounded-xl overflow-hidden animate-slide-in">
      <div class="p-4 flex items-start gap-3">
        ...
        <!-- Content -->
        <div class="flex-1">
          <h4 v-if="title" class="font-bold text-gray-900 mb-1">{{ title }}</h4>
      ...
      <!-- Progress bar -->
      <div v-if="duration" class="h-1 bg-gray-200">
        <div :class="progressBarClass" class="h-full transition-all" :style="{ width: progressWidth + '%' }"></div>
      </div>
    </div>
  </transition>
</template>
