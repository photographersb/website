<template>
  ...
  <nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 md:hidden safe-area-bottom">
    <div class="grid h-16" :class="showVerification ? 'grid-cols-6' : 'grid-cols-5'">
      <!-- Home -->
      <router-link
        to="/"
      ...
      >
        ...
        <span v-if="activeCompetitionsCount > 0" class="absolute top-1 right-1/4 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold">
      ...
      >
        ...
        <!-- Notification Badge -->
        <span v-if="unreadNotifications > 0" class="absolute top-1 right-1/4 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold">
