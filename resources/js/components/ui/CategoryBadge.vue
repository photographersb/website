<template>
  <router-link
    :to="`/photographers/by-category?category=${categorySlug}`"
    :class="[
      'inline-flex items-center gap-2 px-3 py-2 rounded-full font-semibold transition-all duration-200 cursor-pointer',
      sizeClasses[size],
      variantClasses[variant]
    ]"
    :title="`Browse ${categoryName} photographers`"
  >
    <span>{{ categoryName }}</span>
    <svg v-if="showArrow" class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
