...
class CompetitionUpdateRequest extends FormRequest
{
    ...
    {
        $competitionId = $this->route('competition') ?? $this->route('id');
        $isDraft = $this->input('status') === 'draft';

        return [
            'title' => [Rule::requiredIf(!$isDraft), 'nullable', 'string', 'max:255', 'unique:competitions,title,' . $competitionId],
            'slug' => 'nullable|string|max:255|unique:competitions,slug,' . $competitionId,
            ...
            'submission_deadline' => $isDraft ? ['nullable', 'date'] : ['required', 'date'],
